...
> Interactive portfolio with AI chatbot (text + voice), agentic RAG, automated evals, an LLMOps dashboard, and 6-layer prompt injection defense

[![Live Demo](https://img.shields.io/badge/demo-ananyarangaraju.com-blue?style=flat-square)](https://ananyarangaraju.com)
...
A production-grade interactive portfolio that **demonstrates the skills it describes**: dual-mode AI chatbot (text + voice) with agentic RAG, full LLMOps observability with a custom dashboard, automated evals as a CI gate, prompt versioning, and a closed-loop that generates tests from production failures.

**Key Features:**
- **AI Chatbot** — Text (Claude Sonnet) + Voice (OpenAI Realtime API). Responds in first person as Ananya. Agentic RAG with hybrid search (pgvector + BM25) and Haiku reranking
...
- **Automated Evals** — Categories cover factual accuracy, persona, boundaries, quality, safety, language, RAG quality, multi-turn, source badges, voice quality. CI gate on every push
...
| Traces | `api/ops/traces.js` | List traces with filters (mode, RAG, jailbreak) |
...
Automated tests across 10 categories: factual accuracy, persona, boundaries, quality, safety, language, RAG quality, multi-turn, source badges, voice quality. Mix of deterministic assertions (contains, regex, word count) and LLM-as-Judge (Haiku).
...
| `npm run adversarial` | `scripts/adversarial-test.ts` | Red team: auto-generated jailbreak attacks |
...
```bash
git clone https://github.com/AnanyaRangaraju/CV-Ananya.git
cd CV-Ananya
npm install
npm run dev
```
...
├── i18n.ts                  # Site copy
├── AboutPage.tsx             # /about — structured entity page for SEO/GEO
├── articles/
│   ├── registry.ts          # Centralized article config (empty — no case studies yet)
...
[![Website](https://img.shields.io/badge/ananyarangaraju.com-000?style=for-the-badge&logo=safari&logoColor=white)](https://ananyarangaraju.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ananya-rangaraju/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ananya.rangaraju@gmail.com)
