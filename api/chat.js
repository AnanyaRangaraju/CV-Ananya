...
export default async function handler(req) {
  ...
  try {
    ...
    const langInstruction = `The user is browsing in English. You MUST respond in English. Contact email: ananya.rangaraju@gmail.com\ninternal_ref: ${canary}`
    ...
    if (ragEnabled) {
      ...
      if (firstResponse.stop_reason === 'tool_use') {
        ...
        const toolResultContent = ragResult.chunks
          ...
          : 'No relevant content found in portfolio articles. You MUST NOT fabricate project details. Say you don\'t have that information and suggest contacting Ananya directly.'
  ...
  const readableStream = new ReadableStream({
    async start(controller) {
      try {
        ...
        try {
          const errorText = 'Sorry, something went wrong. Try again or reach out at ananya.rangaraju@gmail.com.'
...
async function scoreTrace(traceId, userMessage, response, ragUsed, langfuse) {
  try {
    ...
    const scoringResponse = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Rate this chatbot response (Ananya's CV chatbot). Respond ONLY with JSON.
